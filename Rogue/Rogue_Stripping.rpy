...
label R_Stripping: 
    ...
    while Round >=0:  
        ...
        call RogueLust(1)
        if Count != 2:             
            if ((R_Over and R_Chest) or (R_Over and R_BodySuit)) and (R_Panties or R_Legs or HoseNum("Rogue") >= 10 or R_BodySuit):          #will she lose the overshirt when she's dressed under?
            ...
            elif R_Legs and (R_Panties or HoseNum("Rogue") >= 10 or R_BodySuit):                              #will she lose the pants/skirt if she has panties on?
                if ApprovalCheck("Rogue", 1200, TabM = 3) or (R_BodySuit and ApprovalCheck("Rogue", 900, TabM = 3) and not Taboo) or (R_SeenPanties and ApprovalCheck("Rogue", 900, TabM = 3) and not Taboo):
                    ...
                    if not R_SeenPanties and not R_BodySuit:
            ...
            elif R_Over and not R_Chest and not R_BodySuit and (R_Panties or HoseNum("Rogue") >= 10):      #will she lose the top when she's topless with panties?        
            ...
            elif R_Chest and not R_Over and not R_BodySuit:                                     # Will she lose the bra?
                if ApprovalCheck("Rogue", 1250, TabM = 3) or (R_SeenChest and ApprovalCheck("Rogue", 1000, TabM = 3) and not Taboo):
                    $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 60, 5)                
                    $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 2)
                    ...
                    $ P_Focus = Statupdate("Rogue", "Focus", P_Focus, 80, 15) 
                    $ Line = R_Chest   
                    if R_Chest == "swimsuit1" or R_Chest == "bodysuit" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
            ...
            elif R_Chest and ChestNum("Rogue") == 2 and not R_Over and R_BodySuit:                                     # Will she lose the bra?
                if ApprovalCheck("Rogue", 1250, TabM = 3) or (R_SeenChest and ApprovalCheck("Rogue", 1000, TabM = 3) and not Taboo):
                    $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 60, 5)                
                    $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 2)
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 1)
            ...
            elif R_Chest and ChestNum("Rogue") < 2 and R_BodySuit and R_Panties:                                     # Will she lose the bra?
                if ApprovalCheck("Rogue", 1250, TabM = 3) or (R_SeenChest and ApprovalCheck("Rogue", 1000, TabM = 3) and not Taboo):
                    $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 60, 5)                
                    $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 2)
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 1)
                    ...
                    $ Line = R_BodySuit   
                    $ R_BodySuit = 0                     
                    # if not R_SeenChest or not R_SeenPussy:
                    #     call RogueFace("bemused", 1)
                    #     "She hesitantly glances your way, and then with a shrug takes off her [Line], tossing it to the ground."
                    #     $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 3)                              
                    #     $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 200, 4)
                    #     $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 3)
                    #     $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 200, 3)    
                    ...
                    # else:
                    call RogueFace("sexy")
                    "She takes off her [Line], tossing it to the ground."  
                else:
                    jump R_Strip_Ultimatum

            elif R_Chest and ChestNum("Rogue") < 2 and R_BodySuit:                                     # Will she lose the bra?
                if ApprovalCheck("Rogue", 1250, TabM = 3) or (R_SeenChest and ApprovalCheck("Rogue", 1000, TabM = 3) and not Taboo):
                    $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 60, 5)                
                    $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 2)
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 1)
            ...
            elif R_BodySuit and R_Panties:                                     # Will she lose the bra?
                if ApprovalCheck("Rogue", 1250, TabM = 3) or (R_SeenChest and ApprovalCheck("Rogue", 1000, TabM = 3) and not Taboo):
                    $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 60, 5)                
                    $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 2)
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 1)
            ...
            elif R_BodySuit:                                     # Will she lose the bra?
                if ApprovalCheck("Rogue", 1250, TabM = 3) or (R_SeenChest and ApprovalCheck("Rogue", 1000, TabM = 3) and not Taboo):
                    $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 60, 5)                
                    $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 2)
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 1)
                ...
                if ApprovalCheck("Rogue", 1350, TabM = 3) or (R_SeenPussy and ApprovalCheck("Rogue", 1100, TabM = 3) and not Taboo):
                    $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 75, 10) 
                    $ Line = R_Panties  
                    if R_Panties == "swimsuit1" or R_Panties == "bodysuit" or R_Panties == "swimsuit2":
                        if R_Chest:
                            $ R_Chest = 0
...
label R_Undress(Region = "ask",CountStore=0):  
    ...
    if Region == "ask":
        if R_BodySuit:
            menu:
                "Which parts?"
                "Her top" if R_Over or (R_Chest and ChestNum("Rogue") == 2):    
                    $ Region = "top"     
                "Her bottoms" if R_Legs or R_Hose:
                    $ Region = "bottom"
                "Her body suit" if R_BodySuit:
                    $ Region = "bodysuit"           
                "A little of both. . ." if R_Over or (R_Chest and ChestNum("Rogue") == 2) or R_Legs or R_Hose: 
                    $ Region = "both"    
                "Never mind":
                    pass
    ...
    if Region == "top":
        ...
        if R_Legs or R_Panties or R_Hose:
                call Rogue_Bottoms_Off(0)
    elif Region == "bodysuit":
        if R_BodySuit:
                call Rogue_BodySuit_Off(0)
    elif Region == "both":        
            if R_Over or R_Chest:    
                    call Rogue_Top_Off(0)
            $ Tempmod = CountStore 
...
label Rogue_Top_Off(Intro = 1, Line = 0, Cnt = 0):                                                    # Will she take her top off? Modifiers
    ...
    # if not R_BodySuit:
    #     $ Approval = ApprovalCheck("Rogue", 500, TabM = 4) # 110, 125, 140, 300 taboo, -20 if already seen
    ...
    if Situation == "auto":     
        ...
        if R_Over: # If she's in a top
            if (R_Chest or R_BodySuit) and ApprovalCheck("Rogue", 800, TabM = 1):
            ...
            if not R_Chest and not R_BodySuit:
                if Taboo:
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 90, (int(Taboo/20)))   
                call Rogue_First_Topless(1)
                
        if R_Chest:
            if Approval >= 2 or (R_BodySuit and BodySuitTopCoverage("Rogue") >= 50) or (Approval >= 1 and R_BodySuit) or (R_SeenChest and ApprovalCheck("Rogue", 500) and not Taboo):
                $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 70, 1)
                if Taboo:
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 90, (int(Taboo/20))) 
                if Line:
    ...
    if Approval >= 2 or (R_BodySuit and BodySuitTopCoverage("Rogue") >= 50) or (Approval >= 1 and R_BodySuit): #(R_Love + R_Obed + R_Inbt + (2*Tempmod) - (4*Taboo)) >= 1250:                              # Does she assume top off?            
        ...
        if not R_BodySuit:
            ...
            while ((R_Chest and ChestNum("Rogue") == 2) or R_Over) and Cnt:
                menu:                                                                                 #Menu All off?
                    ch_r "So, [R_Petname]. Did you want me to take my top off?"  
                    "Lose the [R_Over]." if R_Over:                 
                        call RogueFace("bemused", 1)
        ...
        if not R_Chest and not R_Over and not R_BodySuit:             
            $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 1)
            $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 90, 1)
            call Rogue_First_Topless
    ...
    menu:
        ...
        "How about just the [R_Over]?" if R_Over:                                                # asked to go shirtless. 
            if ApprovalCheck("Rogue", 800, TabM = 2) and (R_Chest or R_BodySuit): #80, 160 taboo 
                call RogueFace("sexy")
            ...
            elif not R_Chest and not R_BodySuit:
                $ R_Eyes = "surprised"
                $ R_Blush = 2
        ...
        "Come on, Please? [[take it all off]" if ChestNum("Rogue") == 2:                                                                      # asked to go topless. 110, 270 Taboo           
            if (Approval and ApprovalCheck("Rogue", 600, "L", TabM=1)) or (ApprovalCheck("Rogue", 800, TabM = 2) and R_BodySuit):
...
label Rogue_ToplessorNothing:
    call RogueFace("angry")
    if ApprovalCheck("Rogue", 800, "OI", TabM = 4) and ApprovalCheck("Rogue", 400, "O", TabM = 3) or (R_BodySuit and ApprovalCheck("Rogue", 600, "OI", TabM = 4) and ApprovalCheck("Rogue", 300, "O", TabM = 3)):        
        $ R_Love = Statupdate("Rogue", "Love", R_Love, 20, -2, 1)
        ...
        if R_Over:
            ...
            if R_BodySuit:
                if R_Chest and ChestNum("Rogue") == 2:
                    $ Line = R_Chest
                    if R_Chest == "swimsuit1" or R_Chest == "bodysuit" or R_Chest == "swimsuit2":
                        if R_Panties:
                            $ R_Panties = 0
            ...
            if R_BodySuit and ChestNum("Rogue") == 2:
...
label Rogue_Bottoms_Off(Intro = 1, Line = 0, Cnt = 0):
    ...
    if R_BodySuit and R_BodySuitOff:
        $ R_BodySuit = 0

    if not R_Legs and R_BodySuit:
        call Rogue_BodySuit_Off(Intro, Line, Cnt)
        if R_BodySuit:
            # "returned for wearing bodysuit, ERROR MESSAGE"
            return
    ...
    if Situation == "auto":
        ...
        if not R_Upskirt:                     
            if R_Legs == "skirt" or R_Over == "red dress" or R_Over == "blue dress":                                          #If she's in a skirt with panties, hike it up?
                if Approval >= 2 or (Approval >= 1 and R_BodySuit) or (R_SeenPussy and not Taboo):
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 60, 1)
                    if Taboo:
                        $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 90, (int(Taboo/20)))                 
                    $ R_Upskirt = 1
        ...
        if R_Panties and not R_PantiesDown and not R_BodySuit:                                              # Just wearing panties, lose them?
    ...
    if Approval >= 2 or (R_BodySuit and BodySuitBottomCoverage("Rogue") >= 50) or (Approval >= 1 and R_BodySuit):                 #will she volunteer to strip to underwear?///////////////////////////////////////////////////        
        call RogueFace("sexy", 1)
        if R_Forced:
            call RogueFace("sad", 1)
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 20, -2, 1)
        ...
        if not R_Panties and not R_BodySuit and Action_Check("Rogue", "recent", "bottomless") < 2: 
            $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 1)
            $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 90, 1)
            $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 3)
            $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 80, 3)
        ...
        menu:            
            ...
            "Come on, Please?" if R_Legs or HoseNum("Rogue") >= 10 or (R_Panties == "shorts" and not R_BodySuit):   
            ...
            "It doesn't have to be everything. . ." if R_Legs or HoseNum("Rogue") >= 10 or (R_Panties == "shorts" and not R_BodySuit):    
...
label Rogue_Bottoms_Off_Legs:    
    ...
    $ Cnt = 1
    if R_BodySuit:
        while Cnt and (R_Legs or R_Hose):
            menu:                                       
                # She's asking what you'd like to see.
                ch_r "[Line]"
                
                "Lose the [R_Legs]." if R_Legs: 
                        if ((R_Panties or R_BodySuit) and Approval >= 2) or (R_BodySuit and BodySuitBottomCoverage("Rogue") >= 50) or (Approval >= 1 and R_BodySuit):
                            call RogueFace("sexy")
                            ch_r "I guess I could do that. . ."
                        elif Approval:          
                            call RogueFace("sexy", 1)
                            if Approval < 2 and not R_Panties and not R_BodySuit and HoseNum("Rogue") < 10:
                        ...
                        $ Line = R_Legs      
                        $ R_Legs = 0
                        if not R_Panties and not R_BodySuit and HoseNum("Rogue") < 10:
                            call RogueFace("sly", 2)
                            "She blushes and looks at you slyly before removing her [Line]." 
                            call Rogue_First_Bottomless
                        elif not R_SeenPanties and not R_BodySuit:
                ...
                "Lose the [R_Hose]." if R_Hose:
                        call RogueFace("bemused", 1)
                        if R_Legs or (R_BodySuit and BodySuitBottomCoverage("Rogue") >= 50):
                            ch_r "Ok, no problem."                         
                        elif Approval < 2 and not R_Panties and not R_BodySuit and HoseNum("Rogue") >= 10:
                            call Rogue_NoPanties
                        ...
                        if R_Legs:
                            ...
                            "Rogue pulls her [Line] off." 
                        elif not R_Panties and not R_BodySuit:
                            call RogueFace("sly", 2)
                            "She blushes and looks at you slyly before removing her [Line]." 
                            $ R_Blush = 1
                            call Rogue_First_Bottomless
...
label Rogue_NoPanties: #called when asked to remove pants with nothing on under
    if R_Panties:
        "she has panties, ERROR MESSAGE"
        return
    if not R_Panties:
        "test, she has no panties (this message SHOULD appear)"
    if R_BodySuit:
        "she has a bodysuit, ERROR MESSAGE"
        return
    if (R_Legs or HoseNum("Rogue") >= 10) and not R_BodySuit:
...
label Rogue_BodySuit_Off(Intro = 1, Line = 0, Cnt = 0):
    call Shift_Focus("Rogue")
    
    if not R_BodySuit:                                  # If she's already bodysuitless. Just skip back.     
        $ Tempmod = 0
        return
    ...
    if Intro:
        if R_BodySuit:
                ch_p "This might be easier without your [R_BodySuit] on."
        
    $ Approval = ApprovalCheck("Rogue", 1200, TabM = 5) # 120, 135, 150, -200(320) taboo, -25 if already seen
    
    if Situation == "auto":
        $ Line = 0
        if R_Chest: # If she's in a bra and panties
            ...
            $ Line = R_BodySuit
            if R_Panties:
                "Rogue grumbles to herself, and then removes her [Line], sliding out of it." 
                $ R_SeenPanties = 1
            else:
                "Rogue grumbles to herself, and then removes her [Line], sliding her bare ass out of it." 
                call Rogue_First_Bottomless(1)
            ...
            $ Line = R_BodySuit
            if R_Panties:
                "Rogue grumbles to herself, and then removes her [Line], sliding out of it and showing her breasts." 
                $ R_SeenPanties = 1
            else:
                "Rogue grumbles to herself, and then removes her [Line], sliding her naked body out of it." 
                call Rogue_First_Bottomless(1)
    ...
    if Approval >= 2 or (R_Over and R_Chest) or (Approval >= 1 and (R_Chest or R_Over) or R_SeenChest):                 #will she volunteer to strip to underwear?///////////////////////////////////////////////////        
        if Approval >= 2 or R_Legs or (Approval >= 1 and R_Panties and R_SeenPanties) or R_SeenPussy:                 #will she volunteer to strip to underwear?///////////////////////////////////////////////////        
            
            call RogueFace("sexy", 1)
            menu:
                ch_r "So, [R_Petname]. Should I take my [R_BodySuit] off?" 
                "Yes":
                    pass
            ...
            if not R_Panties and not R_Legs and HoseNum("Rogue") != 10 and Action_Check("Rogue", "recent", "bottomless") < 2: 
                $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 1)
                $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 90, 1)
                $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 3)
                $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 80, 3)
    ...
    elif R_BodySuit:                                                      # She'd rather not strip but might        
        call RogueFace("bemused", 1)
        if "no bodysuitless" in R_RecentActions: 
            call RogueFace("angry")
            ch_r "What did I just tell you, [R_Petname]?"
        elif Approval and not R_SeenChest:
            ch_r "Not everything, right?"   
            ...
            ch_r "I'm not ready to show you all that yet."   
        elif "no bodysuitless" in R_DailyActions: 
            ch_r "Have you forgot what I said earlier, [R_Petname]?"   
        elif Taboo:
            ch_r "I don't know about doing it here. . ."  
        elif Approval:
            ...
            ch_r "I'm not taking my bodysuit off."
        menu:            
            extend ""
            "Ok, never mind." if "no bodysuitless" not in R_RecentActions:  
                if "ask bodysuitless" not in R_DailyActions:
                    $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 80, 2)
                    $ R_Love = Statupdate("Rogue", "Love", R_Love, 70, 1)
                    $ R_Love = Statupdate("Rogue", "Love", R_Love, 90, 1)
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 50, 3)
            ...
            "Sorry, sorry." if "no bodysuitless" in R_RecentActions:  
                ch_r "Ok, fine, just chill out about it."
             
            "Come on, Please?" if R_BodySuit:   
                    if "no bodysuitless" in R_DailyActions:    
                            call RogueFace("angry", 1)
                            ch_r "Listen up when I tell you \"no.\""
                    else:                      
                        if Approval and ApprovalCheck("Rogue", 600, "L", TabM=1):   
                            ...
                            call RogueFace("sexy")
                            call Rogue_BodySuit_Off_Refused
                                    
            "No, lose it.":            #85 and -200 taboo             
                if (Approval and R_Obed >= 250) or (ApprovalCheck("Rogue", 850, "OI", TabM = 5) and ApprovalCheck("Rogue", 400, "O")):                    
                    $ R_Love = Statupdate("Rogue", "Love", R_Love, 20, -1, 1)
                    $ R_Love = Statupdate("Rogue", "Love", R_Love, 70, -5, 1)
                    $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 50, 4)
                    $ R_Inbt = Statupdate("Rogue", "Inbt", R_Inbt, 60, 3)
                    $ Line =  "Fine, if that's what you want."  
                    $ Approval = 1 if Approval < 1 else Approval
                    $ R_Forced = 1
                    $ Line = R_BodySuit
                    $ R_BodySuit = 0
                    ...
                    if ApprovalCheck("Rogue", 400, "O"):
                        ...
                        $ R_RecentActions.append("angry")
                        $ R_DailyActions.append("angry")   
                    $ R_RecentActions.append("no bodysuitless")                      
                    $ R_DailyActions.append("no bodysuitless")   
    
    $ Tempmod = 0
    $ R_RecentActions.append("ask bodysuitless")                      
    $ R_DailyActions.append("ask bodysuitless")     
    return

label Rogue_BodySuit_Off_Refused:     
    if "no bodysuitless" in R_RecentActions:  
        ch_r "What part of \"no\" escapes you, [R_Petname]?"
    elif "no bodysuitless" in R_DailyActions:  
        ch_r "If you keep this up, not ever, [R_Petname]."
    ...
    menu:
        extend ""
        "Sure, never mind." if "no bodysuitless" not in R_RecentActions:
            $ R_Mouth = "smile"
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 70, 2)    
            ch_r "Great!"    
        "Sorry, I'll drop it." if "no bodysuitless" in R_RecentActions:   
            ch_r "Fine. . ."  
        "No, let's do something else.":
            $R_Brows = "confused"
            ch_r "Ok [R_Petname], your loss."               
            $ R_Lust = Statupdate("Rogue", "Lust", R_Lust, 50, 5)
            $ R_Love = Statupdate("Rogue", "Love", R_Love, 70, -2, 1)
            if "no bodysuitless" not in R_RecentActions:  
                $ R_Obed = Statupdate("Rogue", "Obed", R_Obed, 60, 4)      
            $ R_RecentActions.append("angry")
            $ R_DailyActions.append("angry")   
